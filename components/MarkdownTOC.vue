<template>
    <div class="relative">
        <button v-if="isCollapsed" @click="toggleToc"
            class="fixed md:sticky top-5 left-0 md:left-auto z-30 p-2 bg-white rounded-r-md shadow-md border border-l-0 border-gray-200 hover:bg-gray-50 transition-colors"
            aria-label="展开目录">
        ...
        <div v-show="!isCollapsed"
            class="sticky top-5 max-h-[calc(100vh-40px)] overflow-y-auto p-4 border-l bg-white border-gray-200 transition-all duration-300 ease-in-out min-w-[250px]"
            :class="{
            ...
            <div v-if="items.length" class="flex items-center justify-between mb-4">
                <h2 class="font-bold text-lg text-gray-800">目录</h2>
            ...
            <ul class="list-none pl-0 m-0 space-y-1">
                <li v-for="item in items" :key="item.id" :class="`pl-${(item.level - 1) * 4}`"
                    ...
                    <a @click="scrollTo(item.id)"
                        class="block py-1.5 px-2 text-gray-600 no-underline rounded-md transition-colors hover:text-gray-900 hover:bg-gray-50 cursor-pointer whitespace-normal break-words overflow-visible"
                        :class="{
            ...
            <div v-if="items.length === 0" class="text-gray-500 text-sm italic">
                暂无目录项
            </div>
        </div>
    </div>
