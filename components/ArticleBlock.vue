<template>
    <li class="list-none">
        <NuxtLink :to="linkto"
            class="transition-all rounded-xl duration-200 ease-out before:absolute before:inset-y-0 before:z-[-1] before:scale-[0.92] before:rounded-2xl before:opacity-0 before:backdrop-blur before:transition-all before:duration-200 before:origin-[var(--origin-x)_var(--origin-y)] hover:before:scale-100 hover:before:opacity-100 before:bg-(--md-sys-color-primary-container)/40 dark:before:bg-(--md-sys-color-primary-container)/70 relative flex cursor-pointer! flex-col py-6 px-8 before:inset-0  focus-visible:shadow-none! hover:translate-x-2"
            :style="`--origin-x: ${Math.random() * 100}%; --origin-y: ${Math.random() * 30}%`">
            <h2 class="relative wrap-break-word text-2xl font-medium">
                <div class="w-[calc(100%-2rem)] text-(--md-sys-color-on-surface)">{{ title }}</div>
            </h2>
            <div class="absolute top-5 right-6 w-6 h-6" v-if="istop">
                <svg t="1752824665138" class="icon text-(--md-sys-color-primary)" viewBox="0 0 1024 1024" version="1.1"
            ...
            <div class="relative mt-4 space-y-2 text-(--md-sys-color-on-surface-variant)">
                <div class="relative overflow-hidden text-justify">
                    {{ abstext }}
                </div>
            </div>
            <div
                class="post-meta-bar flex select-none flex-wrap items-center justify-end gap-4 text-(--md-sys-color-outline) mt-4">
                <div class="flex min-w-0 shrink grow flex-wrap gap-2 text-sm">
                    <div class="flex min-w-0 items-center space-x-1">
                        <svg width="1em" height="1em" viewBox="0 0 24 24" class="text-(--md-sys-color-primary)">
                            <path fill="currentColor"
                    ...
                    <div class="flex min-w-0 items-center space-x-1" v-if="tags">
                        ...
                        <span class="min-w-0 truncate">
                            <span v-for="(tag, index) in tags" :key="tag">
                                <button
                                    class="shiro-link--underline font-normal hover:text-(--md-sys-color-primary) transition-colors"
                                    tabindex="0">
                                    {{ tag }}
                                </button>
