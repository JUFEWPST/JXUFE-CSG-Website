<template>
  <div class="space-y-3">
    ...
    <div v-if="loading" class="flex items-center justify-center py-4">
      <AnimationLoadingSpinner size="md" color="[var(--md-sys-color-primary)]" />
    </div>
    ...
    <div v-else-if="!events?.length" class="text-xs text-(--md-sys-color-on-surface-variant)">
      {{ t('ctf.empty') }}
...
const statusKey = (e: RawCTFEvent): 'oncoming' | 'running' | 'finished' | 'unknown' => {
  const raw = (e.比赛状态 || '').toLowerCase()
  if (raw === 'oncoming') return 'oncoming'
  if (raw === 'running') return 'running'
  if (raw === 'finished' || raw === 'over' || raw === 'done') return 'finished'
  return 'unknown'
}
...
const iconForStatus = (status: 'oncoming' | 'running' | 'finished' | 'unknown') => iconMap[status]

const iconColorClass = (status: 'oncoming' | 'running' | 'finished' | 'unknown') => {
  switch (status) {
    case 'running':
...
const fetchEvents = async () => {
  ...
  error.value = null
  try {
    const res = await fetch('https://raw.githubusercontent.com/ProbiusOfficial/Hello-CTFtime/main/Global.json')
    if (!res.ok) {
